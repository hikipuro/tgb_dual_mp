...
void mbc::ext_write(word adr,byte dat)
{
	...
	char op_name[][20]={"prefix","write","read","delete"};
	char pre_op_name[][20]={"cant write","write all addr","delete all addr","can delete"};
	int i;
...
void mbc::mbc1_write(word adr,byte dat)
{
	if (mbc1_16_8){//16/8モード
		...
		case 1:
			...
			//printf("offset: %02x\n", 0x4000*((mbc1_dat==0?1:mbc1_dat)&(rom_size_tbl[ref_gb->get_rom()->get_info()->rom_size]-1))-0x4000);
			//printf("size: %02x\n", ref_gb->get_rom()->get_info()->rom_size);
			//printf("mbc1_dat: %02x\n", mbc1_dat);
			//printf("rom_page: %02x\n", *rom_page);
			break;
		case 2:
