...
public:
	...
	void write(word adr,byte dat);
	word readw(word adr) { return read(adr)|(read(adr+1)<<8); }
	void writew(word adr,word dat) { write(adr,(byte)dat);write(adr+1,dat>>8); }
...
private:
	byte io_read(word adr);
	void io_write(word adr,byte dat);
